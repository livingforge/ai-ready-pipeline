...
- `python .github/skills/specdb/scripts/engine.py` — 検証レポート + 統計（error で exit 1）
- `python .github/skills/specdb/scripts/generate.py [文書名]` — 設計書の生成
- `python .github/skills/specdb/scripts/diff.py <ベースライン>` / `--baselines` — 差分レポート
- `python .github/skills/specdb/scripts/visualize.py` — 対話型グラフビューア
- `python .github/skills/docextract/scripts/run_docagent.py {facts|facts-stats|facts-export|search|get|text|list} ...`
  — ファクト・出典の**参照のみ**（fact-add 等の書き込み系は使わない）
- データルートの初期化に限り `cp -r .github/skills/specdb/scaffold .specdb` 相当のコピーを行ってよい
...
1. **データルートの確認・初期化** — `.specdb/`（`metamodel.yaml` を持つ）があるか確認。
   無ければ scaffold（`.github/skills/specdb/scaffold/`）をコピーして初期化し、サンプルの
