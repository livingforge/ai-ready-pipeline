...
依存は PyYAML + Jinja2 のみ（`.github/skills/specdb/scripts/requirements.txt`）:
...
雛形 `.github/skills/specdb/scaffold/` をプロジェクトへコピーして開始する:

```bash
cp -r .github/skills/specdb/scaffold .specdb     # .specdb なら --root の指定を省略できる
```
...
```bash
python .github/skills/specdb/scripts/engine.py                    # 検証レポート + 統計
python .github/skills/specdb/scripts/generate.py                  # 全文書を out/ に生成
python .github/skills/specdb/scripts/generate.py table-spec       # 指定文書だけ生成
python .github/skills/specdb/scripts/diff.py     baseline/R1.0    # ベースライン差分
python .github/skills/specdb/scripts/diff.py     --baselines      # ベースライン一覧
python .github/skills/specdb/scripts/visualize.py                 # 対話型ビューア out/specdb.html
```
...
名前空間）の詳細は `.github/skills/specdb/scaffold/README.md` と
`.github/skills/specdb/scaffold/metamodel.yaml` のコメントを参照。
...
同梱テストで動作確認できる: `python -m pytest .github/skills/specdb/scripts/tests -q`
