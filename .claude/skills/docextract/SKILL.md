...
```bash
python .claude/skills/docextract setup --check   # state only, changes nothing, no approval needed
...
`python .claude/skills/docextract extract ...` instead — same interface.
