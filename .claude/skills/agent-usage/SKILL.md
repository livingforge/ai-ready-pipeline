...
python .claude/skills/agent-usage/scripts report --out out/

# 直近 30 日、特定プロジェクトだけ、JSON だけ
python .claude/skills/agent-usage/scripts report --days 30 --project ai-ready-pipeline --json-only

# 期間指定・別の Claude ディレクトリ・別単価表
python .claude/skills/agent-usage/scripts report --since 2026-06-01 --until 2026-06-30 \
...
python .claude/skills/agent-usage/scripts report --agent copilot --out out-copilot/

# 特定ワークスペースだけ（フォルダパス指定。id 直指定は --workspace-id）
python .claude/skills/agent-usage/scripts report --agent copilot --workspace C:/proj/foo --out out-copilot/
```
